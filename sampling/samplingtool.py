...
# --- 4. ヘルパー関数 ---
def get_marker_center(target_id, detected_ids, detected_corners):
    """検出されたマーカーから、指定IDのマーカーの中心座標(ピクセル)を返す"""
...
# --- 5. メイン処理 ---
cap = cv2.VideoCapture(0) # 0番のカメラを使用 (適宜変更)
if not cap.isOpened():
...
while True:
    ...
    # 4隅のマーカーの中心座標を取得
    src_pts = [get_marker_center(id, detected_ids, detected_corners) for id in CORNER_IDS]
    ...
    if all(pt is not None for pt in src_pts):
        ...
        # 筆マーカー(ID 4)の中心座標を取得
        pen_center_pixel = get_marker_center(PEN_ID, detected_ids, detected_corners)
